...
public interface BookSpringDataJdbcRepository extends CrudRepository<Book, Integer> {
   ...
   @Query("SELECT * FROM book " +
      "WHERE LENGTH(title) = (SELECT MIN(LENGTH(title)) FROM book)")
   ...
   CompletableFuture<Stream<Book>> findBooksByIdBetweenAsync(@Param("fromId") Integer from, @Param("toId") Integer to);
}
